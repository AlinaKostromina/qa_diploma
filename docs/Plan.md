...
5. Результат - окно с сообщением "Успешно! Операция добрена банком!", в БД появилась запись со статусом APPROVED.
...
1. Заполнить данные карты тестовыми данными:
    ...
    * CVC/CVV: 453
4. Кликнуть на поле "Продолжить"
5. Результат - текст в поле "Номер карты" "Поле обязательно для заполнения", в БД запись не появилась.
...
1. Заполнить данные карты тестовыми данными:
    ...
    * CVC/CVV: 456
4. Кликнуть на поле "Продолжить"
5. Окно с ошибкой. Текст "Банк отказал в проведении операции", в БД запись не появилась.
...
_Сценарий 5._  
_Отправить заявку с недостаточным количеством цифр в номере карты дебетовая карта (менее 16 цифр)._
1. Зайти на сайт http://localhost:8080/
1. Кликнуть на поле "Купить"
1. Заполнить данные карты тестовыми данными:
    ...
    * CVC/CVV: 043
4. Кликнуть на поле "Продолжить"
5. Результат - текст в поле "Номер карты" "Неверный формат", в БД запись не появилась.

_Сценарий 6._  
_Отправить заявку с недостаточным количеством цифр в номере карты кредитная карта (менее 16 цифр)._
1. Зайти на сайт http://localhost:8080/
1. Кликнуть на поле "Купить в кредит"
1. Заполнить данные карты тестовыми данными:
   * номер карты: 4444 4444 4444 444
...
_Сценарий 7._  
_Некорректное написание месяца действия карты (1 цифра) дебетовая карта._
1. Зайти на сайт http://localhost:8080/
1. Кликнуть на поле "Купить"
1. Заполнить данные карты тестовыми данными:
    * номер карты: 4444 4444 4444 4441
    ...
    * CVC/CVV: 503
4. Кликнуть на поле "Продолжить"
5. Результат - текст в поле "Месяц": "Неверный формат", в БД запись не появилась.

_Сценарий 8._  
_Некорректное написание месяца действия карты (1 цифра) кредитная карта._
1. Зайти на сайт http://localhost:8080/
1. Кликнуть на поле "Купить в кредит"
1. Заполнить данные карты тестовыми данными:
   * номер карты: 4444 4444 4444 4441
...
_Сценарий 9._  
_Невалидный месяц действия карты дебетовая карта (значение больше 12)._
1. Зайти на сайт http://localhost:8080/
1. Кликнуть на поле "Купить"
1. Заполнить данные карты тестовыми данными:
    * номер карты: 4444 4444 4444 4441
    ...
    * CVC/CVV: 456
4. Кликнуть на поле "Продолжить"
5. Результат - текст в поле "Месяц" "Неверно указан срок действия карты", в БД запись не появилась.

_Сценарий 10._  
_Невалидный месяц действия карты кредитная карта (значение больше 12)._
1. Зайти на сайт http://localhost:8080/
1. Кликнуть на поле "Купить в кредит"
1. Заполнить данные карты тестовыми данными:
   * номер карты: 4444 4444 4444 4441
...
1. Заполнить данные карты тестовыми данными:
    ...
    * CVC/CVV: 456
4. Кликнуть на поле "Продолжить"
5. Результат - текст в поле "Месяц" "Поле обязательно для заполнения", в БД запись не появилась.
...
_Сценарий 15._  
_Просроченный срок действия карты дебетовая карта (значение меньше текущего года)._
1. Зайти на сайт http://localhost:8080/
1. Кликнуть на поле "Купить"
1. Заполнить данные карты тестовыми данными:
    * номер карты: 4444 4444 4444 4441
    ...
    * CVC/CVV: 008
4. Кликнуть на поле "Продолжить"
5. Результат - текст в поле "Год" "Неверно указан срок действия карты", в БД запись не появилась.

_Сценарий 16._  
_Просроченный срок действия карты кредитная карта (значение меньше текущего года)._
1. Зайти на сайт http://localhost:8080/
1. Кликнуть на поле "Купить в кредит"
1. Заполнить данные карты тестовыми данными:
   * номер карты: 4444 4444 4444 4441
...
_Сценарий 19._  
_Отправить заявку с некорректным значением в поле "Год" дебетовая карта (1 цифра)._
1. Зайти на сайт http://localhost:8080/
1. Кликнуть на поле "Купить"
1. Заполнить данные карты тестовыми данными:
    * номер карты: 4444 4444 4444 4441
    ...
    * CVC/CVV: 024
4. Кликнуть на поле "Продолжить"
5. Результат - текст в поле "Год": "Неверный формат", в БД запись не появилась.

_Сценарий 20._  
_Отправить заявку с некорректным значением в поле "Год" кредитная карта (1 цифра)._
1. Зайти на сайт http://localhost:8080/
1. Кликнуть на поле "Купить в кредит"
1. Заполнить данные карты тестовыми данными:
   * номер карты: 4444 4444 4444 4441
...
1. Заполнить данные карты тестовыми данными:
    ...
    * CVC/CVV: 430
4. Кликнуть на поле "Продолжить"
5. Результат - текст в поле "Владелец" : "Поле обязательно для заполнения", в БД запись не появилась.
...
1. Заполнить данные карты тестовыми данными:
    ...
    * CVC/CVV: 021
4. Кликнуть на поле "Продолжить"
5. Результат - текст в поле "Владелец" : "Неверный формат", в БД запись не появилась.
...
1. Заполнить данные карты тестовыми данными:
    ...
    * CVC/CVV:
4. Кликнуть на поле "Продолжить"
5. Результат - текст в поле "CVC/CVV" "Поле обязательно для заполнения", в БД запись не появилась.
...
_Сценарий 31_  
_Отправить заявку с невалидным значением в поле: "CVC/CVV" дебетовая карта (1 цифра)._
1. Зайти на сайт http://localhost:8080/
1. Кликнуть на поле "Купить".
1. Заполнить данные карты тестовыми данными:
    * номер карты: 4444 4444 4444 4441
    ...
    * CVC/CVV: 9
4. Кликнуть на поле "Продолжить"
5. Результат - текст в поле "CVC/CVV" "Неверный формат", в БД запись не появилась.

_Сценарий 32_  
_Отправить заявку с невалидным значением в поле: "CVC/CVV" кредитная карта (1 цифра)._
1. Зайти на сайт http://localhost:8080/
1. Кликнуть на поле "Купить в кредит".
1. Заполнить данные карты тестовыми данными:
   * номер карты: 4444 4444 4444 4441
...
1. Результат - текст "Поле обязательно для заполнения", в БД запись не появилась.
...
_Сценарий 35_  
_Отправить заявку на покупку тура с невалидным заполнением полей (кириллица) дебетовая карта._
1. Зайти на сайт http://localhost:8080/
1. Кликнуть на поле "Купить".
1. Заполнить данные карты тестовыми данными:
   * номер карты: 4444 4444 4444 4441
...
_Сценарий 36_  
_Отправить заявку на покупку тура с невалидным заполнением полей (кириллица) кредитная карта._
1. Зайти на сайт http://localhost:8080/
1. Кликнуть на поле "Купить в кредит".
1. Заполнить данные карты тестовыми данными:
   * номер карты: 4444 4444 4444 4441
...
_Сценарий 37_  
_Прописать невалидный номер карты дебетовая карта._
1. Зайти на сайт http://localhost:8080/
1. Кликнуть на поле "Купить".
1. Заполнить данные карты тестовыми данными:
   ...
   * CVC/CVV: 002
4. Кликнуть на поле "Продолжить"
5. Результат - окно с сообщением "Ошибка! Банко отказал в проведении операции!", в БД появилась запись со статусом DECLINED.

_Сценарий 38_  
_Прописать невалидный номер карты кредитная карта._
1. Зайти на сайт http://localhost:8080/
1. Кликнуть на поле "Купить в кредит".
1. Заполнить данные карты тестовыми данными:
   * номер карты: 4444 4444 4444 4442
...
11. 11. **Faker** - библиотека для генерации даннных, помогает получить данные для тестов.
